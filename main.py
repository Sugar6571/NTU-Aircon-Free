"""
ESP32 MicroPython AC-FREE UART IR controller.
...
BLINKER_FAN_MODES = ("fan_1", "fan_2", "fan_3", "fan_4", "fan_5", "strong")
...
def fan_mode_to_slider(fan_mode):
    if fan_mode == "strong":
        return 6
    if fan_mode in BLINKER_FAN_MODES:
        return BLINKER_FAN_MODES.index(fan_mode) + 1
    return 1


def slider_to_fan_mode(value):
    try:
    ...
    if fan_value > 6:
        fan_value = 6
    return BLINKER_FAN_MODES[fan_value - 1]
...
def main():
    ...
    ap_password = setup_cfg.get("ap_password", "yzc202657")

    wifi = WifiManager(config.get("device_name", "ntu_ac_controller"), ap_password)
    setup_mode = False
    setup_ip = ""
    time_synced = False
    wifi_status = "--"
    if config_missing_wifi(config):
        setup_mode = True
        setup_ip = wifi.start_ap(ap_password)
        wifi_status = "AP"
    ...
    web = WebConfigServer(config, save_config, web_state)
    ...
    def blinker_state_dict():
        ...
        fan_value = fan_mode_to_slider(fan_mode)
        avg_temp = sensor.average_temperature()
        state = {}
        ...
        state[ids.get("current_temperature", "num-d2v")] = {
            "value": float(fmt_number(avg_temp, 1)) if avg_temp is not None else 0
        }
    ...
    def handle_blinker_json(data):
        ...
        for key in data:
            ...
            if key == power_id or low in ("switch", "power", "pwr", "acfree", "ac_free"):
                ...
                changed.append("tex-status")
            elif key == fan_id or (str(key).startswith("ran-") and numeric_value(value) is not None and numeric_value(value) <= 6):
                climate.set_fan_mode(slider_to_fan_mode(value))
                changed.append(fan_id)
                changed.append("tex-status")
            elif key == temp_id or (str(key).startswith("ran-") and numeric_value(value) is not None and numeric_value(value) >= 18):
    ...
    if config.get("ble", {}).get("enabled", True):
        ids = config.get("blinker", {})
        sync_commands = [
            '{"rt":["' + ids.get("power", "btn-0fh") + '"]}',
            '{"rt":["' + ids.get("fan", "ran-5hc") + '"]}',
            '{"rt":["' + ids.get("target_temperature", "ran-egl") + '"]}',
            '{"rt":["' + ids.get("current_temperature", "num-d2v") + '","' + ids.get("humidity", "num-m74") + '"]}',
            '{"rt":["tex-status"]}',
        ]
        ble = BleRemote(config.get("ble", {}).get("name", "Blinker"), web_state, handle_ble_command, sync_commands)
        ble.start()
    ...
    direct_blow_window_ms = int(climate_cfg.get("direct_blow_window_ms", 30000))
    direct_blow_drop_c = float(climate_cfg.get("direct_blow_drop_c", 1.5))
