...
TEMP_MIN_C = 18.0
TEMP_MAX_C = 32.0
DEFAULT_TARGET_C = 25.0
DEFAULT_FAN_MODE = "strong"
...
class ClimateController:
    ...
    def _ensure_defaults(self):
        if "climate" not in self.config:
            self.config["climate"] = {}
        climate = self.config["climate"]
        try:
            self.set_target_temperature(climate.get("target_temperature", DEFAULT_TARGET_C))
        except Exception:
            climate["target_temperature"] = DEFAULT_TARGET_C
        if climate.get("fan_mode") not in FAN_MODES:
            climate["fan_mode"] = DEFAULT_FAN_MODE
    ...
    def adjust_fan_mode(self, delta):
        ...
        try:
            idx = FAN_MODES.index(current)
        except ValueError:
            idx = 0
        idx = (idx + delta) % len(FAN_MODES)
        self.set_fan_mode(FAN_MODES[idx])
    ...
    def update(self, current_temperature=None, direct_blow=False):
        ...
        now = time.ticks_ms()
        if time.ticks_diff(now, self.last_keepalive_ms) >= KEEPALIVE_INTERVAL_MS and not self.ir.busy():
