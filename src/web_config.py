...
class WebConfigServer:
    def __init__(self, config, save_config, climate, oled, command_path, get_state=None):
        self.config = config
    ...
    def index_page(self):
        ...
        body = (
            "<h1>AC-FREE Setup</h1>"
            "<div class='card'><b>Time</b><div class='time'>" + str(state.get("time", "--:--")) + "</div>"
            "<p class='muted'>Wi-Fi is only used for NTP time display. AC control uses OLED buttons and Blinker BLE.</p></div>"
            "<div class='card'><b>Wi-Fi</b><p>Status: " + str(state.get("wifi", "--")) + "</p>"
            "<p>IP: " + str(state.get("ip", "192.168.4.1")) + "</p><p>2.4GHz only.</p></div>"
            "<div class='card'><b>Provisioning</b><form method='POST' action='/save'>"
            "<label>Device name<input name='device_name' value='" + self.config.get("device_name", "") + "'></label>"
            "<label>Wi-Fi SSID<input name='ssid' value='" + wifi.get("ssid", "") + "'></label>"
            "<label>Wi-Fi password<input name='wifi_password' type='password' value='" + wifi.get("password", "") + "'></label>"
